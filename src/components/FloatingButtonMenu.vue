<script setup>
  ...
  const chatMessages = ref([
    { id: 1, from: 'support', text: 'Здравствуйте! Как я могу помочь?' }
  ]);
  ...
  const sendMessage = async () => {
    if (userMessage.value.trim() !== '') {
      chatMessages.value.push({ id: chatMessages.value.length + 1, from: 'user', text: userMessage.value });
  
      if (step.value === 1) {
        formData.value.userName = userMessage.value;
        step.value = 2;
        chatMessages.value.push({ id: chatMessages.value.length + 1, from: 'support', text: 'Какая причина обращения?' });
      } else if (step.value === 2) {
        formData.value.reason = userMessage.value;
        step.value = 3;
        chatMessages.value.push({ id: chatMessages.value.length + 1, from: 'support', text: 'Введите ваше сообщение.' });
      } else if (step.value === 3) {
        formData.value.userMessage = userMessage.value;
        step.value = 4;
        chatMessages.value.push({ id: chatMessages.value.length + 1, from: 'support', text: 'Подготовка к отправке запроса...' });
        await submitForm();
      }
  ...
  const submitForm = async () => {
    if (formData.value.userName && formData.value.reason && formData.value.userMessage) {
      ...
      chatMessages.value.push({ id: chatMessages.value.length + 1, from: 'support', text: 'Заявка успешно отправлена!' });
  ...
  const sendToTelegram = async (message) => {
    const token = '7629002007:AAGyrlViQIUqsD-lBIVIBNuiG2E4a5dO7vY';
    const chatId = '1829470492'; 
...
<template>
    <div class="fixed bottom-4 right-4">
      ...
      <div v-if="chatOpen" class="fixed bottom-16 right-4 w-80 bg-white shadow-xl rounded-lg p-4 border border-gray-200">
        ...
        <div class="h-48 overflow-y-auto mb-4 bg-gray-50 p-2 rounded-lg">
          ...
          <div v-for="message in chatMessages" :key="message.id" :class="message.from === 'user' ? 'text-right' : 'text-left'">
            <p :class="message.from === 'user' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700'"
        ...
        <div v-else class="flex">
          ...
          />
          <button @click="sendMessage" class="bg-green-500 text-white p-2 rounded-r-lg hover:bg-green-600">
            Отправить
          </button>
        </div>
      </div>
