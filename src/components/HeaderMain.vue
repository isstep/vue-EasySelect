...
const headerClasses = computed(() => ({
  ...
  'shadow-md backdrop-blur-sm bg-white/90': isSticky.value, 
  ' border-b border-transparent': !isSticky.value,
...
<template>
  <header :class="headerClasses">
    <div
      ...
      :class="{ 'max-h-0 opacity-0 border-none': isCollapsed, 'max-h-10 opacity-100 py-1': !isCollapsed }"
    >
      <div class="flex items-center justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-sm">
        <span class="text-gray-600 flex items-center gap-2">
          <img class="h-4 w-4" src="/public/123.svg" alt="Location" /> <!-- Use a better icon -->
          <span class="text-blue-600 font-medium hover:underline cursor-pointer">Укажите адрес</span>
          <span class="text-gray-500">Сегодня, 10:00 - 21:00</span>
        </span>
        <nav class="flex items-center space-x-4 text-gray-600">
          <a href="#" class="hover:text-emerald-600 transition-colors">Доставка и оплата</a>
          <a href="#" class="hover:text-emerald-600 transition-colors">Контакты</a>
          <a href="#" class="hover:text-emerald-600 transition-colors">Для юр. лиц</a>
          <a href="#" class="hover:text-emerald-600 transition-colors">Вакансии</a>
          <a href="#" class="hover:text-emerald-600 transition-colors">EasySelect.by</a>
        </nav>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16">
          ...
          <transition
           ...
           >
            ...
            >
              ...
              >
                <img :src="result.imgUrl || '/placeholder-image.svg'" alt="" class="w-8 h-8 rounded object-cover mr-3 flex-shrink-0" />
      ...
      <nav class="flex items-center gap-3 sm:gap-4 lg:gap-5 flex-shrink-0">
        ...
        <div class="relative">
          ...
          >
            ...
            <span class="hidden lg:inline text-sm font-medium">{{ userDisplayName }}</span>
             <svg v-if="authStore.isAuthenticated" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 hidden lg:inline ml-1 transition-transform duration-200" :class="{'rotate-180': isProfileDropdownOpen}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <button class="lg:hidden text-gray-500 hover:text-gray-700 focus:outline-none  focus:ring-inset  ">
...
/* Add subtle animation for the badge */
@keyframes pulse {
...
input:focus-visible {
     outline: none; /* Ring handles focus */
}
/* Specific focus for close button if you add one */
